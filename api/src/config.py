...
config = AsyncOrmProvider(
    db_type="postgresql",
    config_params=dict(
        host=os.getenv("PG_HOST_NAME", "localhost"),
        db_name=os.getenv("PG_DB", "default_nest_db"),
        user=os.getenv("PG_USERNAME", "postgres"),  
        password=os.getenv("PG_PASSWORD", "postgres"),
        port=int(os.getenv("PG_CONTAINER_PORT", 5432)),
    )
)

# METRICS
INFO = Gauge(
    "pynest_app_info", "PyNest application information.", [
        "app_name"]
)
REQUESTS = Counter(
    "pynest_requests_total", "Total count of requests by method and path.", [
        "method", "path", "app_name"]
)
RESPONSES = Counter(
    "pynest_responses_total",
    "Total count of responses by method, path and status codes.",
    ["method", "path", "status_code", "app_name"],
)
REQUESTS_PROCESSING_TIME = Histogram(
    "pynest_requests_duration_seconds",
    "Histogram of requests processing time by path (in seconds)",
    ["method", "path", "app_name"],
)
EXCEPTIONS = Counter(
    "pynest_exceptions_total",
    "Total count of exceptions raised by path and exception type",
    ["method", "path", "exception_type", "app_name"],
)
REQUESTS_IN_PROGRESS = Gauge(
    "pynest_requests_in_progress",
